...
def test_get_returns_none_when_the_cache_is_empty():
    ...
    result = cache.get([1.0, 0.0], TIER_EMPLOYEE, MODEL, similarity_threshold=0.95, ttl_seconds=300)
...
def test_get_returns_the_cached_answer_for_an_identical_embedding():
    cache = SemanticCache()
    cache.put([1.0, 0.0], TIER_EMPLOYEE, MODEL, _answer("Arsenal won 2-1."))
...
def test_get_preserves_citations_for_a_cache_hit():
    ...
    cache = SemanticCache()
    cache.put([1.0, 0.0], TIER_EMPLOYEE, MODEL, _answer("Arsenal won 2-1. [1]", [_CITATION]))
...
def test_get_returns_the_cached_answer_for_a_similar_enough_embedding():
    ...
    result = cache.get([0.99, 0.01], TIER_EMPLOYEE, MODEL, similarity_threshold=0.95, ttl_seconds=300)
...
def test_get_returns_none_below_the_similarity_threshold():
    ...
    result = cache.get([0.0, 1.0], TIER_EMPLOYEE, MODEL, similarity_threshold=0.95, ttl_seconds=300)
...
def test_get_picks_the_most_similar_entry_among_several():
    cache = SemanticCache()
    cache.put([0.9, 0.1], TIER_EMPLOYEE, MODEL, _answer("weakly similar answer"))
    cache.put([0.0, 1.0], TIER_EMPLOYEE, MODEL, _answer("orthogonal answer"))
    cache.put([1.0, 0.0], TIER_EMPLOYEE, MODEL, _answer("closest match"))

    result = cache.get([1.0, 0.0], TIER_EMPLOYEE, MODEL, similarity_threshold=0.9, ttl_seconds=300)
...
def test_get_raises_on_mismatched_embedding_dimensions_for_the_same_model():
    cache = SemanticCache()
    cache.put([1.0, 0.0, 0.0], TIER_EMPLOYEE, MODEL, _answer("answer"))

    with pytest.raises(ValueError):
        cache.get([1.0, 0.0], TIER_EMPLOYEE, MODEL, similarity_threshold=0.95, ttl_seconds=300)
...
def test_get_ignores_entries_cached_under_a_different_embedding_model():
    ...
    cache = SemanticCache()
    cache.put([1.0, 0.0, 0.0], TIER_EMPLOYEE, "other-model", _answer("wrong-model answer"))
...
def test_get_ignores_entries_older_than_the_ttl():
    cache = SemanticCache()
    cache.put([1.0, 0.0], TIER_EMPLOYEE, MODEL, _answer("stale answer"), now=1000.0)

    result = cache.get(
        [1.0, 0.0], TIER_EMPLOYEE, MODEL, similarity_threshold=0.95, ttl_seconds=60, now=1070.0
    )
...
def test_get_returns_entries_still_within_the_ttl():
    cache = SemanticCache()
    cache.put([1.0, 0.0], TIER_EMPLOYEE, MODEL, _answer("fresh answer"), now=1000.0)

    result = cache.get(
        [1.0, 0.0], TIER_EMPLOYEE, MODEL, similarity_threshold=0.95, ttl_seconds=60, now=1030.0
    )
...
def test_cosine_similarity_is_clamped_to_the_valid_range():
    ...
    cache = SemanticCache()
    vector = [0.1] * 768
    cache.put(vector, TIER_EMPLOYEE, MODEL, _answer("self-match answer"))

    result = cache.get(vector, TIER_EMPLOYEE, MODEL, similarity_threshold=1.0, ttl_seconds=300)
...
def test_answer_with_cache_runs_the_full_pipeline_on_a_cache_miss(
    ...
    answer = answer_with_cache(
        "Who won?", TIER_EMPLOYEE, cache=cache, embedding_cache=EmbeddingCache(), **KWARGS
    )
...
def test_answer_with_cache_does_not_cache_across_different_tiers(
    ...
    answer = answer_with_cache(
        "Who won?", TIER_MANAGER, cache=cache, embedding_cache=EmbeddingCache(), **KWARGS
    )
...
def test_answer_with_cache_passes_query_and_user_tier_through_to_the_pipeline(
    ...
    assert mock_plan.call_args.kwargs["user_tier"] == TIER_MANAGER
