...
def test_get_returns_the_cached_answer_for_an_identical_embedding():
    cache = SemanticCache()
    cache.put([1.0, 0.0], "tier-1", MODEL, "Arsenal won 2-1.")
...
def test_get_never_returns_an_entry_cached_under_a_different_user_tier():
    ...
    cache = SemanticCache()
    cache.put([1.0, 0.0], "tier-1", MODEL, "tier-1's answer.")
...
def test_get_picks_the_most_similar_entry_among_several():
    cache = SemanticCache()
    cache.put([0.9, 0.1], "tier-1", MODEL, "weakly similar answer")
    cache.put([0.0, 1.0], "tier-1", MODEL, "orthogonal answer")
    cache.put([1.0, 0.0], "tier-1", MODEL, "closest match")
...
def test_get_raises_on_mismatched_embedding_dimensions_for_the_same_model():
    cache = SemanticCache()
    cache.put([1.0, 0.0, 0.0], "tier-1", MODEL, "answer")
...
def test_get_ignores_entries_cached_under_a_different_embedding_model():
    ...
    cache = SemanticCache()
    cache.put([1.0, 0.0, 0.0], "tier-1", "other-model", "wrong-model answer")
...
def test_get_ignores_entries_older_than_the_ttl():
    cache = SemanticCache()
    cache.put([1.0, 0.0], "tier-1", MODEL, "stale answer", now=1000.0)
...
def test_get_returns_entries_still_within_the_ttl():
    cache = SemanticCache()
    cache.put([1.0, 0.0], "tier-1", MODEL, "fresh answer", now=1000.0)
...
def test_cosine_similarity_is_clamped_to_the_valid_range():
    ...
    cache = SemanticCache()
    vector = [0.1] * 768
    cache.put(vector, "tier-1", MODEL, "self-match answer")

    result = cache.get(vector, "tier-1", MODEL, similarity_threshold=1.0, ttl_seconds=300)

    assert result == "self-match answer"
...
def test_answer_with_cache_skips_the_pipeline_on_a_cache_hit(
    ...
    mock_embed.return_value = [[1.0, 0.0]]
    cache = SemanticCache()
    cache.put([1.0, 0.0], "tier-1", MODEL, "Arsenal won 2-1. [1]")
...
def test_answer_with_cache_populates_the_cache_after_a_miss(
    ...
    result = cache.get([1.0, 0.0], "tier-1", MODEL, similarity_threshold=0.95, ttl_seconds=300)
    assert result == "Arsenal won 2-1. [1]"
...
def test_answer_with_cache_does_not_cache_an_answer_containing_the_fallback_even_when_sufficient(
    ...
    mock_generate.return_value = (
        f" {CANNOT_ANSWER_MESSAGE} [1] does not contain information about that."
    )
    cache = SemanticCache()
...
def test_answer_with_cache_does_not_cache_across_different_tiers(
    ...
    mock_embed.return_value = [[1.0, 0.0]]
    cache = SemanticCache()
    cache.put([1.0, 0.0], "tier-1", MODEL, "tier-1's cached answer")
    mock_plan.return_value = _insufficient_result()
    mock_generate.return_value = CANNOT_ANSWER_MESSAGE
